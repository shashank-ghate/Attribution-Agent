...
function App() {
  ...
  const isMoeReady = health?.moengage_mode === 'mock' || (moengage.status === 'connected' && moengage.profile_id === selectedMoengageProfile)
    ...
    <header className="border-b border-slate-200/80 bg-white/90 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-[1500px] items-center justify-between px-5 lg:px-8">
        ...
        <div className="flex items-center gap-3">
          ...
          <Badge tone={health?.moengage_mode === 'mock' ? 'amber' : 'green'}>{health?.moengage_mode === 'mock' ? 'DEMO' : 'BROWSER MODE'}</Badge>
        </div>
      </div>
    </header>
      ...
      <section className="mb-5 grid gap-4 lg:grid-cols-2">
        ...
        <div className={`rounded-2xl border bg-white p-6 shadow-sm ${isMoeReady ? 'border-emerald-200' : 'border-slate-200'}`}>
          ...
          <div className="mt-5 rounded-xl bg-slate-50 p-4">
            ...
            <p className="text-xs leading-5 text-slate-600">{health?.moengage_mode === 'mock' ? 'Mock mode is enabled for development.' : isMoeReady ? moengage.message : 'Choose a login profile, open the automation browser, and complete Google login. The session is saved only for that profile.'}</p>
            <div className="mt-3 flex flex-wrap gap-2">{health?.moengage_mode !== 'mock' && <button onClick={startAutomationBrowser} disabled={busy === 'moengage' || busy === 'reset-moengage' || !moengageProfile.trim() || !moengagePassword || Boolean(job)} className="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-xs font-semibold text-white disabled:opacity-50"><Icon name="browser" className="size-4" />{busy === 'moengage' ? 'Signing in…' : 'Login with this profile'}</button>}{health?.moengage_mode !== 'mock' && <button onClick={verifyAutomationBrowser} disabled={busy === 'verify' || moengage.profile_id !== selectedMoengageProfile} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700 disabled:opacity-50"><Icon name="refresh" className="size-4" />Verify login</button>}{health?.moengage_mode !== 'mock' && <button type="button" onClick={resetAutomationBrowser} disabled={busy === 'reset-moengage' || !moengageProfile.trim() || !moengagePassword || Boolean(job)} className="inline-flex items-center gap-2 rounded-lg border border-red-200 bg-white px-4 py-2 text-xs font-semibold text-red-600 disabled:opacity-50"><Icon name="close" className="size-4" />{busy === 'reset-moengage' ? 'Resetting…' : 'Clear profile & login'}</button>}{health?.moengage_mode !== 'mock' && <a href={moengageDashboardUrl} target="_blank" rel="noopener noreferrer" onClick={openMoEngage} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-slate-700"><Icon name="link" className="size-4" />Open manually</a>}</div>
          </div>
        </div>
      </section>
            ...
            <div className="flex-1">
              ...
              : job?.status === 'processing' ? <button onClick={stopAutomation} className="rounded-xl border border-red-200 px-4 py-2 text-xs font-semibold text-red-600">Stop run</button>
                : job && terminalStates.has(job.status) ? <button onClick={() => setJob(null)} className="rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-semibold text-blue-700">Start another run</button>
                  : !isMoeReady ? <Badge tone="amber">CONNECT MOENGAGE TO RUN</Badge> : null}
          </div>
        </section>
